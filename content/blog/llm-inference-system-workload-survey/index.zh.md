...
[下载中文论文 PDF（v1.1）](/blog/llm-inference-system-workload-survey/survey-zh-v1.1.pdf)

*论文版本 v1.1，日期 2026-09-07。*
...
| Zhou 等[[77]](#ref-zhou2024efficient) | 优化层次 | – | ○ | ○ |
| **本文** | **工作负载** | ● | ● | ● |
...
本文与两类相邻工作的分工是： 与评测方法学讨论[[14]](#ref-agrawal2025evaluating)相比，
该工作面向实践者列举评估中的常见问题，本文侧重负载本身的描述框架与公开数据的梳理；
与单个数据集的刻画工作[[20]](#ref-xiang2026servegen), [[21]](#ref-wang2025burstgpt), [[15]](#ref-wang2025kvcachewild)相比，
...
把两阶段部署在不同设备上称为**预填解码分离**[[1]](#ref-patel2024splitwise), [[35]](#ref-zhong2024distserve)；
把预填充切分成若干块与解码交替执行称为**分块预填充**[[33]](#ref-agrawal2023sarathi), [[34]](#ref-agrawal2024sarathiserve)。
...
**连续批处理**[[32]](#ref-yu2022orca)指在每个解码步重新组织批次，
已完成的请求离开批次，新到达的请求随时加入。 批次的组成因此取决于请求的到达时刻与剩余输出长度，
两者都是负载的属性。
在此基础上，若干引擎进一步把预填充与解码在批次内合并调度[[42]](#ref-holmes2024fastgen)，或以最大化吞吐为目标重排批内算子[[43]](#ref-zhu2024nanoflow)。
与之相对，面向离线批量处理的系统[[44]](#ref-sheng2023flexgen)不受时延约束，
...
| BurstGPT*[[21]](#ref-wang2025burstgpt) | 高校服务 | 1 秒整数 | 无 | 部分文件有 | 各文件列数不一致 |
| Mooncake*[[3]](#ref-qin2025mooncake) | 月之暗面 | 毫秒字段（实测约 3 秒） | 块编号，块大小 512 | 无 | 分对话、工具、合成三类 |
| ServeGen*[[20]](#ref-xiang2026servegen) | 阿里 | 600 秒速率表 | 块哈希，块大小 16 | 多轮文件有 | 在线与多轮两份 |
| ACDC*[[19]](#ref-yang2026acdc) | 阿里 | 无（整批提交） | 块编号，块大小 16 | 无 | 离线批处理，六份 |
| 百炼轨迹[[15]](#ref-wang2025kvcachewild) | 阿里 | 相对时刻，绝对时刻已移除 | 块哈希，块大小 16 | 有父子链 | 公开的是两小时抽样 |
...
| *（c）前端对话日志，非服务端* |  |  |  |  |  |
| WildChat[[76]](#ref-zhao2024wildchat) | 聊天前端 | 微秒绝对时刻，未量化 | 原文 | 有 | 无词元数、无排队信息 |
| ShareGPT* | 用户分享 | 无 | 原文 | 有 | 事实上的默认长度语料 |
| LMSYS-Chat-1M*[[75]](#ref-zheng2024lmsys) | 对话竞技场 | 无 | 原文 | 有 | 本体无时间戳字段 |
...
亿请求的服务商数据[[71]](#ref-nixon2026year)
（其脚注声明发布已获批准，但截至本文检索时未见发布）、 一份覆盖 320 万用户与 7.61
亿次调用的代码补全服务数据[[66]](#ref-liu2026copilot)， 以及若干声明将开源但仓库不存在的工件。
这些数据若能发布，将明显改变上表的格局。
...
并给出了按拓扑条件化的间隔分布[[70]](#ref-lamagna2026multiagent)。
...
DistServe[[35]](#ref-zhong2024distserve)与
FastServe[[36]](#ref-wu2023fastserve)采用了相同的处理理由， 其中 FastServe
明确标注沿用先前工作。 三篇论文的措辞高度接近，但本文只能观察到这一理由的重复出现，
不足以判定其传播路径：FastServe 与 DistServe 存在作者重叠，
各版本的时间先后也需核对版本历史才能确定。

作为对照，AlpaServe[[37]](#ref-li2023alpaserve)的处理是把原始轨迹切成时间窗，
...
以负载均衡与迁移为目标的调度工作[[38]](#ref-sun2024llumnix)依赖实例间瞬时负载的差异，
而该差异由到达的突发程度决定；
面向流式输出体验的工作[[39]](#ref-liu2024andes)把用户感知的流畅度定义为随时间变化的量，
其评估结论同样依赖请求在时间上的分布；
无服务器形态的推理服务[[40]](#ref-fu2024serverlessllm)以冷启动开销为核心问题，
...
一类工作试图预测长度以改进调度。
$S^3$[[56]](#ref-jin2023s3)训练分类器预测输出长度区间，据此分配显存并提高批次占用率；
序列调度[[57]](#ref-zheng2023seqsched)用模型自身预知回复长度， 把长度相近的请求归入同一批次。
两者都要求给出预测误差的处理方式，因为低估会导致重新分配。

另一类工作不预测长度，而是让系统对长度的不确定性不敏感。
SuperServe[[58]](#ref-khare2023superserve)面向不可预知负载调整模型规模；
尾部感知调度[[23]](#ref-beyondpred2026)明确以不依赖预测为前提， 针对尾时延而非平均时延设计。
这两类工作的前提互斥：前者的收益随预测精度上升，后者的价值恰在预测不可靠时体现。
本文未见二者在同一负载下的直接对照。

长度分布的形态还决定某些系统形态是否成立。
以检索增强与推荐为代表的场景中，输出长度极短甚至只有一个词元，
针对这类负载的引擎[[59]](#ref-du2025prefillonly)把解码阶段整体略去；
反之，长上下文工作[[54]](#ref-wu2024loongserve), [[55]](#ref-lin2024infinitellm)
...
SMetric[[16]](#ref-wang2026smetric)指出，被评测模型的回复与轨迹记录不同会以两种方式 破坏 KV
...
一项 2026 年的能耗刻画工作[[22]](#ref-vellaisamy2026energy)报告：
...
延迟指标上存在类似情况。一项 2026 年的调度工作[[23]](#ref-beyondpred2026)报告，
...
**按前缀组织缓存。**
SGLang[[45]](#ref-zheng2024sglang)以基数树组织已缓存的前缀，使多条请求共享公共部分；
提示缓存[[46]](#ref-gim2024promptcache)进一步允许复用非连续的模块化片段，
代价是需要预先声明可复用的结构。
两者的收益都随请求间共享比例上升，而该比例是负载的属性而非系统的属性。

**把复用纳入调度。** Preble[[49]](#ref-srivatsa2025preble)把前缀共享作为分布式调度的目标，
使同前缀请求落在同一实例；
MemServe[[50]](#ref-hu2024memserve)在分离式架构下用弹性内存池跨实例共享上下文。
这类工作的评估结论对负载的共享结构最为敏感： 若测试负载中请求彼此独立，调度策略之间不会出现差别。

**放宽“完全相同”的要求。** CacheBlend[[47]](#ref-yao2025cacheblend)针对检索增强场景，
允许把多个非前缀位置的缓存片段融合后使用，并对少量位置重新计算；
Cache-Craft[[51]](#ref-agarwal2025cachecraft)以分块方式管理检索片段的缓存。
这两项工作说明，可复用的范围取决于对生成质量损失的容忍度，
因而“复用率”这一数值本身依赖于所采用的复用判据。
CacheGen[[48]](#ref-liu2024cachegen)则把已计算的缓存压缩后跨节点传输，
其收益取决于缓存被再次使用的概率。

**改变缓存的存储形态。**
vAttention[[52]](#ref-prabhu2025vattention)指出分页并非实现动态显存管理的唯一途径，
主张用虚拟内存机制保持张量连续。 这一路线与块大小的关系不同于分页方案， 因而第
[6.4](#sec-threeerr) 节所述的块粒度偏差在该形态下的表现需要另行分析。
关于键值缓存管理的更完整梳理见相关综述[[53]](#ref-li2026kvsurvey)。
...
a[[19]](#ref-yang2026acdc)（原始块大小 16）为基准做粗化实验： 将相邻的若干个 16
...
TraceLab[[17]](#ref-zhu2026tracelab)测得的每轮追加长度中位数为 875 词元， 相当于 1.7 个 512
块或 13.7 个 64 块。
...
7,955 至 19,019 词元， Sarathi[[33]](#ref-agrawal2023sarathi)使用的论文摘要数据集输入 P90 为
12,985 词元。
...
围绕这一点已有若干系统工作： 以分离式专家并行提升规模效率[[60]](#ref-zhu2025megascaleinfer)、
按专家激活模式在多节点间划分[[63]](#ref-bambhaniya2026moeactivation)、
以及预测专家激活以提前搬运权重[[61]](#ref-yu2025moepatterns)。
这些工作的收益都取决于路由是否集中于少数专家。

需要注意的是，路由的可预测性本身存在争议。
一项分析[[62]](#ref-wang2026moemyth)认为专家的分工反映表示空间的几何结构，
...
篇为面向真实系统的工作，另一篇为仿真器
（Frontier[[78]](#ref-feng2026frontier)的有状态请求抽象含思考轮次与工具调用时延）。 补充样本的
8 篇中有 1 篇建模了多轮会话（TokenSim[[79]](#ref-wu2025tokensim)）。
...
CacheWise[[18]](#ref-tiwari2026cachewise)是本文样本中处理最完整的一例。
...
生产平台的多轮会话分析[[16]](#ref-wang2026smetric)、
编码智能体的缓存行为[[18]](#ref-tiwari2026cachewise)、
智能体负载的总体特征[[65]](#ref-yuan2026agentic)，
以及代码补全服务的大规模轨迹刻画[[66]](#ref-liu2026copilot)。 后者报告了 320 万用户与 7.61
亿次调用的规模，但未发布轨迹。
另有工作从协调拓扑的角度建模多智能体系统的流量[[70]](#ref-lamagna2026multiagent)，
其结论是到达间隔应按拓扑条件化，而非用单一分布刻画。

**利用依赖结构的系统工作。** 一类工作把工作流的可预测性作为调度依据[[64]](#ref-yu2026pythia)，
即在请求到达之前根据已知的流程结构预取或预留资源；
另一类关注智能体负载对存储带宽的压力[[69]](#ref-wu2026dualpath)，
其动因是会话状态的反复换入换出。 这两类工作都要求负载具备真实的依赖结构， 用独立请求流无法评估。

**评测工具与仿真。** 面向智能体负载的基准[[68]](#ref-wang2026xperf)与
多轮会话仿真器[[67]](#ref-rajib2026agentservesim)在同一时期出现。
...
按下游任务分离是较早的一类。 一项工作[[41]](#ref-hu2024interference)指出，
当摘要类与对话类请求混合在同一实例上时，两者相互干扰， 因而主张按下游负载分离部署——
该主张的前提正是流量存在可区分的类别。
多模态场景中也有类似做法[[73]](#ref-papaioannou2026tcmserve)，
按模态调度以应对不同模态在计算量上的差异。

适配器维度的多路复用是另一类。 面向多适配器环境的工作[[74]](#ref-iliakopoulou2024chameleon)
按适配器的使用频率自适应地缓存与调度。 这类工作与本文第五个方面相邻但不相同：
适配器共享同一基础模型，而模型路由指向不同的基础模型， 两者在显存占用与切换代价上的量级不同。

生产测量方面，FineServe[[72]](#ref-zhang2026fineserve) 按架构、规模与任务意图三个维度分层，
...
抽取方式参照系统文献综述的通行做法[[31]](#ref-kitchenham2007guidelines)：
...
另有两篇接近而未做：TokenSim[[79]](#ref-wu2025tokensim)将块粒度仿真列为其精度来源，
...
Scheduling[[57]](#ref-zheng2023seqsched)，
该文的研究问题直接涉及按预测长度截断的机制，因而必须交代。

$S^3$[[56]](#ref-jin2023s3) 的情况需要分开看：该文以输出长度预测为主题，
...
在补充样本的 4 个仿真器（LLMServingSim[[80]](#ref-cho2024llmservingsim)、
TokenSim[[79]](#ref-wu2025tokensim)、APEX+、SplitwiseSim）中，
...
SGLang[[45]](#ref-zheng2024sglang)报告吞吐时“运行足够大的一批程序实例以计算最大吞吐”，
...
Preble[[49]](#ref-srivatsa2025preble)在开环设置下评测同类机制， 报告相对其自建的 SGLang 基线有
...
Frontier[[78]](#ref-feng2026frontier)在四种负载上做保真度验证：
三个固定的输入输出长度点（2048/256、256/2048、1024/1024）与一个 ShareGPT 轨迹。
...
同年的 SMetric[[16]](#ref-wang2026smetric)在某生产轨迹上测得 KV 复用超过请求词元的 80%， 其中
65% 以上来自同一会话的后续请求。
...
到达速率仅在附录中出现一次（ShareGPT 轨迹以每秒 64 请求回放）。 对一个以时序保真为目标的仿真器，
主实验不交代到达过程本身值得注意。
...
**原文的可得性。** Orca[[32]](#ref-yu2022orca)一文的实验设置一节未能获取：其全文为 USENIX
...
**推理服务综述。**
已有综述[[8]](#ref-miao2024towards), [[9]](#ref-zhen2025taming), [[10]](#ref-park2025engines), [[11]](#ref-li2024hpec), [[12]](#ref-pan2025survey), [[77]](#ref-zhou2024efficient)
以服务机制为组织主线，其覆盖情况见表 [1](#tab-surveycmp)。

**负载选择的影响。** Papaioannou 与
Doudali[[13]](#ref-papaioannou2024workload)指出多数系统在评估中使用合成数据集，
并报告文本生成与摘要类负载之间约三倍的吞吐差异。 该文是本文检索范围内最早明确讨论这一问题的工作，
篇幅为八页研讨会论文，涉及单一系统，未涉及轨迹分析与分类框架。

**评测方法讨论。** Agrawal
等人[[14]](#ref-agrawal2025evaluating)整理了推理服务评估中的常见问题，
按基线公平性、评估设置与指标设计三方面组织， 其中包括未能代表生产场景的负载选择。
该工作面向实践者，本文侧重负载本身的描述框架，两者关注点不同。

**生产轨迹刻画。** Wang 等人[[15]](#ref-wang2025kvcachewild)报告了某大型云服务商的 KV
...
包括真实编码智能体轨迹[[17]](#ref-zhu2026tracelab)、
生产平台的多轮会话分析[[16]](#ref-wang2026smetric)
与会话级缓存管理研究[[18]](#ref-tiwari2026cachewise)。

**负载生成。** ServeGen[[20]](#ref-xiang2026servegen)基于生产平台的客户端分解建模，
是本文考察的通用负载生成器中参数化程度最高的； BurstGPT[[21]](#ref-wang2025burstgpt)以 Gamma
过程刻画突发性并发布了长期轨迹。

**早期的负载刻画方法。** 本文的方法参照更早的网络与云计算测量文献： Arlitt 与
Williamson[[24]](#ref-arlitt1997web)的跨轨迹不变量抽取、 Crovella 与
Bestavros[[25]](#ref-crovella1997selfsimilar)对统计现象追溯产生机制的做法、 Barford 与
Crovella[[26]](#ref-barford1998surge)关于合成负载生成器保真度的论证、 Shahrad
等人[[27]](#ref-shahrad2020serverless)与 Joosen 等人[[28]](#ref-joosen2023howdoes)
先发布并刻画、再作纵向重访的做法， 以及 Schroeder
等人[[5]](#ref-schroeder2006open)关于开闭环负载模型的讨论。 评测报告规范方面参照
Heiser[[29]](#ref-heiser2025crimes) 与 Hoefler、Belli[[30]](#ref-hoefler2015scientific)。
